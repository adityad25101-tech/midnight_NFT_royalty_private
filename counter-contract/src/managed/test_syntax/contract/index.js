...
export class Contract {
  ...
  constructor(...args_0) {
    ...
    this.circuits = {
      test_fn: (...args_1) => {
        if (args_1.length !== 1) {
          throw new __compactRuntime.CompactError(`test_fn: expected 1 argument (as invoked from Typescript), received ${args_1.length}`);
        }
        const contextOrig_0 = args_1[0];
        if (!(typeof(contextOrig_0) === 'object' && contextOrig_0.currentQueryContext != undefined)) {
          __compactRuntime.typeError('test_fn',
                                     'argument 1 (as invoked from Typescript)',
                                     'test_syntax.compact line 9 char 1',
                                     'CircuitContext',
                                     contextOrig_0)
        }
        ...
        const result_0 = this._test_fn_0(context, partialProofData);
        partialProofData.output = { value: [], alignment: [] };
        return { result: result_0, context: context, proofData: partialProofData, gasCost: context.gasCost };
      }
    };
    this.impureCircuits = { test_fn: this.circuits.test_fn };
  }
  initialState(...args_0) {
    if (args_0.length !== 1) {
    ...
    state_0.setOperation('test_fn', new __compactRuntime.ContractOperation());
  ...
  _test_fn_0(context, partialProofData) {
    const tmp_0 = 1n;
    __compactRuntime.queryLedgerState(context,
                                      partialProofData,
                                      [
